...
use reactive_graph::{
    owner::Owner, signal::ArcRwSignal, wrappers::read::MaybeSignal,
};
use std::hash::Hash;
use tachys::{
    reactive_graph::OwnedView,
    view::keyed::{keyed, keyed_enumerate},
};
...
///         key=|counter| counter.id
///         // renders each item to a view
///         children=move |index: MaybeSignal<usize>, counter: Counter| {
///           view! {
...
where
    IF: Fn() -> I + Send + 'static,
    I: IntoIterator<Item = T> + Send + 'static,
    EF: Fn(MaybeSignal<usize>, T) -> N + Send + Clone + 'static,
    N: IntoView + 'static,
    KF: Fn(&T) -> K + Send + Clone + 'static,
    K: Eq + Hash + 'static,
    T: Send + 'static,
{
    ...
    let children = move |index, child| {
        let owner = parent.with(Owner::new);
        let (set_index, view) = owner.with(|| {
            let (index, set_index) = ArcRwSignal::new(index).split();
            (set_index, children(index.into(), child))
        });
        (set_index, OwnedView::new_with_owner(view, owner))
    };
    move || keyed_enumerate(each(), key.clone(), children.clone())
}
